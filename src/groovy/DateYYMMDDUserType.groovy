...
public class DateYYMMDDUserType implements UserType {
	...
	private Date convertIntegerToDate(BigDecimal someDate){
		String dateStr = someDate.toString()
		if (dateStr.size() == 5) {
			dateStr = "0"+dateStr
		}
		Date date = new Date();
		try {
			date = dateFormatYYMMDD.parse(dateStr);
		} catch (Exception e) {
			e.printStackTrace()
			println e
		}
	...
	private BigDecimal convertDateToInteger(Date someDate) {
		...
		try {
			decDate = new BigDecimal(dateFormatYYMMDD.format(someDate));
		} catch (Exception e) {}
		return decDate;
	}
}
