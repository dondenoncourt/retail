...
public class DateMMDDYYUserType implements UserType {

	def SQL_TYPES = [Hibernate.STRING.sqlType()];
	private static SimpleDateFormat dateFormatMMDDYY = new SimpleDateFormat("MMddyy");
	...
	private Date convertIntegerToDate(BigDecimal someDate){
		Date date = new Date();
		try {
			def dateMMDDYY = someDate.toString();
			if (dateMMDDYY.size() == 5) {
				dateMMDDYY = '0'+dateMMDDYY;
			}
			date = dateFormatMMDDYY.parse(dateMMDDYY);
		} catch (Exception e) {
			e.printStackTrace()
			println e
		}
	...
	private BigDecimal convertDateToInteger(Date someDate) {
		...
		try {
			decDate = new BigDecimal(dateFormatMMDDYY.format(someDate));
		} catch (Exception e) {}
		return decDate;
	}
}
